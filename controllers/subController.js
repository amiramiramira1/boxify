...
const getSubscriptionsByUser = async (req, res) => {
  ...
  try {
    const subscriptions = await Subscription.find({ user: userId })
      .populate("box")
      .populate("user"); // Populate the box and user fields with their respective data
...
const createSubscription = async (req, res) => {
  try {
    ...
    if (existingSubscription) {
      return res
        .status(400)
        .json({ message: "Subscription already exists for this user and box" });
    }
...
const updateSubscription = async (req, res) => {
  ...
  try {
    let subscription = await Subscription.findByIdAndUpdate(subId, req.body, {
      new: true,
    })
      .populate("box")
      .populate("user"); // Find the subscription by id and populate box and user
    if (!subscription) {
      return res.status(404).json({ message: "Subscription not found" });
    }
